...
# Ensure input path ends with a trailing slash for clean string replacement
if (-not $resolvedInput.EndsWith([System.IO.Path]::DirectorySeparatorChar)) {
    $resolvedInput += [System.IO.Path]::DirectorySeparatorChar
}
...
foreach ($file in $jsonFiles) {
    ...
    # Calculate relative path safely via string replacement
    $relativePath = $file.FullName.Replace($resolvedInput, "")
